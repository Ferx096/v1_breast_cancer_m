...
# ================================
ruta_guardado = r"C:\Users\grupo\OneDrive\Escritorio\MODELO_PCA_CANCER_MAMA\Codigo\arch_descomprimidos\imagenes_y_etiquetas_balanceadas4.pkl"
...
# Importar modelos:
den = DenseNet121(weights="imagenet", input_shape=input_shape)
extractor_den = Model(inputs=den.input, outputs=den.get_layer("avg_pool").output)  # extraer 1280 caracteristicas
# Extraer caracteristicas de las imagenes
caracteristicas_den = extractor_den.predict(imagenes, batch_size=32, verbose=1)
print(caracteristicas_den.shape)


# GUARDAR CARACTERISTICAS EXTRAUDAS
#ruta_den = r"C:\Users\grupo\OneDrive\Escritorio\Nueva carpeta\prueba_umbrales\den_umbrales\caracteristicas_densenet21.pkl"
#with open(ruta_den, "wb") as f:
#    pickle.dump(caracteristicas_den, f)
#print("Caracteristicas guardadas en:", ruta_den)
...
features_den_norm = scaler.fit_transform(caracteristicas_den)
...
# DF
df_im = pd.DataFrame(
    {"Caracteristica": [f"Feature_{i}" for i in range(caracteristicas_den.shape[1])], "IM": im}
)
