...
# Crear modelo Inception_v4 preentrenado
inc = create_model("inception_v4", pretrained=True)
# Remover la parte superior del modelo y usar la capa `flatten` como salida
extractor_inc = nn.Sequential(*(list(inc.children())[:-1]))
extractor_inc.add_module("flatten", nn.Flatten())

print(
    f"Modelo configurado con capa de extracción: {list(extractor_inc.children())[-1]}"
)  # Verificar la capa de extracción
# Configurar el dispositivo
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
extractor_inc = extractor_inc.to(device)
...
# Cargar imágenes (asumiendo que `imagenes` es un arreglo numpy previamente preparado)
imagenes_tensor = torch.tensor(
    imagenes.transpose(0, 3, 1, 2)
)  # Convertir a formato PyTorch (N, C, H, W)
caracteristicas_inc = extraer_caracteristicas(extractor_inc, imagenes_tensor)
print(f"Shape de características extraídas: {caracteristicas_inc.shape}")

# GUARDAR CARACTERISTICAS EXTRAIDAS
ruta = r"C:\Users\grupo\OneDrive\Escritorio\Nueva carpeta\prueba_umbrales\inc_umbrales\caracteristicas_inceptionv8.pkl"
with open(ruta, "wb") as f:
    pickle.dump(caracteristicas_inc, f)
...
features_inc_norm = scaler.fit_transform(caracteristicas_inc)

# =================================
# 4. CALCULAR INFORMACIÓN MUTUA
# =================================
im = mutual_info_classif(features_inc_norm, etiquetas, random_state=42)
df_im = pd.DataFrame(
    {"Característica": [f"Feature_{i}" for i in range(caracteristicas_inc.shape[1])], "IM": im}
)
...
for umbral in umbrales:
    ...
    caracteristicas_reducidas = features_inc_norm[:, indices_seleccionados]
