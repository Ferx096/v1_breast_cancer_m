...
# Importar modelos:
mob = MobileNetV3Large(weights="imagenet", input_shape=input_shape)
extractor_mob = Model(inputs=mob.input, outputs=mob.get_layer("flatten").output)  # extraer 1280 caracteristicas
# Extraer caracteristicas de las imagenes
caracteristicas_mob = extractor_mob.predict(imagenes, batch_size=32, verbose=1)
extractor_mob.summary()
print(caracteristicas_mob.shape)
# GUARDAR CARACTERISTICAS EXTRAUDAS
#ruta_mob = r"C:\Users\grupo\OneDrive\Escritorio\Nueva carpeta\prueba_umbrales\mob_umbrales\caracteristicas_mobileNetV3Large.pkl"
#with open(ruta_mob, "wb") as f:
#    pickle.dump(caracteristicas_mob, f)
#print("Caracteristicas guardadas en:", ruta_mob)
...
features_mob_norm = scaler.fit_transform(caracteristicas_mob)
...
im = mutual_info_classif(features_mob_norm, etiquetas, random_state=42)
# DF
df_im = pd.DataFrame(
    {"Caracteristica": [f"Feature_{i}" for i in range(caracteristicas_mob.shape[1])], "IM": im}
)
...
for umbral in umbrales:
    ...
    caracteristicas_reducidas = features_mob_norm[
